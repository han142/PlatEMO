...
% 创建: agent = DDPG(state_dim, action_dim, action_bound)
%       agent = DDPG(..., optOverrides)  
% 动作: action = agent.selectAction(state, explore)
% 经验: agent.store(state, action, reward, next_state, done)
% 训练: [cl, al] = agent.train()
% 超参数: agent.opts
    ...
    methods
        function obj = DDPG(state_dim, action_dim, action_bound, optOverrides)
            if nargin < 4, optOverrides = struct(); end
        ...
        function action = selectAction(obj, state, explore)
            ...
            if explore
                ...
                om = obj.opts.ou_mu;
                obj.ou_state = obj.ou_state + ot * (om - obj.ou_state) ...
                    + os * randn(1, obj.action_dim);
        ...
        function [critic_loss, actor_loss] = train(obj)
            ...
            [critic_loss, gradC] = dlfeval( ...
                @ddpgCriticGradients, obj.critic, dlarray(sa, 'CB'), dlarray(y_row, 'CB'));
            [obj.critic, obj.avgGradCritic, obj.avgSqCritic] = adamupdate( ...
                obj.critic, gradC, obj.avgGradCritic, obj.avgSqCritic, obj.adamIter, obj.opts.lr_critic);
            
            s_tr = single(states');
            [actor_loss, gradA] = dlfeval( ...
                @ddpgActorGradients, obj.actor, obj.critic, dlarray(s_tr, 'CB'), obj.action_bound);
            [obj.actor, obj.avgGradActor, obj.avgSqActor] = adamupdate( ...
                obj.actor, gradA, obj.avgGradActor, obj.avgSqActor, obj.adamIter, obj.opts.lr_actor);
