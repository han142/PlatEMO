    ...
    methods
        ...
        function PopObj = CalObj(obj,PopDec)
            ...
            for i=1:obj.M
                ...
                temp(:,1) = temp(:,1)+1.*(abs(PopDec(:,3)+.6)+abs(PopDec(:,4)+.6));
                temp(:,2) = temp(:,2)+2.*(abs(PopDec(:,3)+.2)+abs(PopDec(:,4)+.2));
                temp(:,3) = temp(:,3)+3.*(abs(PopDec(:,3)-.2)+abs(PopDec(:,4)-.2));
                temp(:,4) = temp(:,4)+4.*(abs(PopDec(:,3)-.6)+abs(PopDec(:,4)-.6));
                PopObj(:,i) = min(temp,[],2);
            end             
        end
